...
@section('content')
    <style>
        ...
        .one-third__products_list {
            display: flex;
            flex-wrap: wrap;
            gap: 5px; /* Added gap property */
        }

        .one-third__products_product {
            width: calc(33.33% - 5px); /* Adjusted width to account for gap*/
            margin-top: 0; /* Remove individual margin-top as gap handles spacing*/
        ...
        .grid-container {
            display: grid;
            grid-template-columns: repeat(auto-fit, minmax(160px, 1fr)); /* Adjust minmax as needed */
            gap: 16px; /* Adjust gap as needed */
            padding: 16px; /* Add padding to the container, if needed */
        }

        .grid-item {
            display: flex; /* Use flexbox for vertical alignment within grid items */
            flex-direction: column; /* Stack image and text vertically */
            align-items: center; /* Center content horizontally */
            /* The width and height are now handled by the grid-template-columns */
        }

        .grid-item .overflow-hidden { /* Style the image container */
            width: 100%; /* Match the original width */
            height: 100%; /* Match the original height */
            overflow: hidden;
        }
    ...
    <div class="home-banner-area mb-3">
        <div class="container">
            <div class="row gutters-12 position-relative">
                ...
                @if(count($featured_products) > 0)
                    <div class="col-12 col-lg-3 mt-4">
                        <div class="todays_deal bg-white rounded-2 overflow-hidden">
                            ...
                            <div class="c-scrollbar-light overflow-auto p-2 bg-primary" style="height: auto">

                                <div class="one-third__products_list">
                                    @foreach ($featured_products as $key => $product)
                                        <div class="one-third__products_product">
                                            <div class="base-img" style="overflow: hidden;position: relative;width: 100%;height: 110px;">
                                                <a href="{{ route('product', $product->slug) }}" style="padding-bottom:133.33%;" class="base-img__href" role="link">
                                                    <img style="object-position:initial;height: 100%;vertical-align: top;width: 100%;object-fit: cover;"
                             class="lazyload base-img__cover fsp__one-third-products base-img__inner fsp-element"
                             src="{{ static_asset('assets/img/placeholder.jpg') }}"
                             data-src="{{ uploaded_asset($product->thumbnail_img) }}"
                             alt="{{ $product->getTranslation('name') }}" fetchpriority="auto"></a></div>
                                            <div class="price-mask">
                                                <div class="price-wrap not-fsp-element"><span
                                                        style="font-size:9px;margin-left:1px;">{{ home_discounted_base_price($product) }}</span>
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
                    ...
                    @if (get_setting('home_slider_images', null, $lang) != null)
                        <div class="home-slider">
                            <div class="aiz-carousel overflow-hidden rounded-2" data-autoplay="true"
                                 data-infinite="true" data-arrows="true"
                                 data-dots="false">
                                @php
                                    ...
                                    <div class="carousel-box">
                                        <a class="d-block"
                                           href="{{ isset(json_decode($home_slider_links, true)[$key]) ? json_decode($home_slider_links, true)[$key] : '' }}">
                                            <img
                                                class="d-block mw-100 img-fit h-180px h-md-320px  h-lg-300px"
                ...
                <!-- Today's deal -->
                @php
                    $todays_deal_products = filter_products(App\Models\Product::where('todays_deal', '1'))->orderBy('id', 'desc')->limit(6)->get();
                @endphp
                        ...
                        @foreach ($featured_categories as $key => $category)
                            ...
                            <div class="grid-item">
                                <div class="d-flex flex-column align-items-center overflow-hidden">
                                    ...
                                    <p class="mt-1 mb-0 fs-12 text-center text-truncate-2 px-2">
