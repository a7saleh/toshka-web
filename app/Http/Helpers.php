...
if (!function_exists('get_featured_products')) {
    function get_featured_products($limit = 12)
    {
        return Cache::remember('featured_products' . $limit, 3600, function () use ($limit) {
            $product_query = Product::query();
            return filter_products($product_query->where('featured', '1'))->latest()->limit($limit)->get();
        });
    }
}
